 ...
 * standard Calendar functionality to change dates etc. Beware when calculating
 * twilight with a deep dip below the horizon for locations as far south of the
 * North Pole as London, in the northern hemisphere. The sun never reaches this
 * dip at certain times of the year. When the calculations encounter this
 * condition a null will be returned when a <code>{@link java.util.Date}</code>
 * is expected and {@link Long#MIN_VALUE} when a long is expected. The reason
 * that <code>Exception</code>s are not thrown in these cases is because the
 * lack of a rise/set or twilight is not an exception, but expected in many
 * parts of the world.
 *
 * Here is a simple example of how to use the API to calculate sunrise: <br />
 * First create the Calendar for the location you would like to calculate:
 *
 * String locationName = &quot;Melbourne, Australia&quot;;
 *
 * double latitude = -37.45; // for Melbourne, AU
 *
 * double longitude = 145; // for Melbourne, AU
 *
 * double elevation = 10; // optional elevation in Meters
 ...
 * </pre>
 *
 * @author Rajesh Vasa, 2011 Code cleaned up based on work by Eliyahu Hershfeld
 *         to support Android
 */
public class AstronomicalCalendar
{
	...
	private SunTimesCalculator astronomicalCalculator;
