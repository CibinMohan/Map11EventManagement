package com.example.c4cib.map1111.calc;
 ...
 * required for astronomical calculations. Elevation value is used if provided.
 * 
 */
public class GeoLocation
{
	 ...
	 *            Lakewood, NJ <br/>
	 *            <b>Note: </b> For latitudes south of the equator, a negative
	 ...
	 *            would be used for Lakewood, NJ. <b>Note: </b> For latitudes
	 *            south of the equator, a negative value should be used.
	 */
	 ...
	 * @param minutes
	 *            <a href=
	 *            "http://en.wikipedia.org/wiki/Minute_of_arc#Cartography"
	 *            >minutes of arc</a>
	 * @param seconds
	 *            <a href=
	 *            "http://en.wikipedia.org/wiki/Minute_of_arc#Cartography"
	 ...
	 * set in the {@link AstronomicalCalendar}, it is
	 * critical that
	 ...
	 * ever {@link AstronomicalCalendar#clone() cloned}.
	 * 
	 * @param timeZone
	 *            The timeZone to set.
	 */
	 ...
	 * {@link AstronomicalCalendar#getSunTransit() noon}
	 * (with adjustment for the <a
	 * href="http://en.wikipedia.org/wiki/Equation_of_time">equation of
	 * time</a>) the sun should be directly overhead, so a user who is 1&deg;
	 * west of this will have noon at 4 minutes after standard time noon, and
	 * conversely, a user who is 1&deg; east of the 15&deg longitude will have
	 * noon at 11:56 AM. The offset returned does not account for the <a
	 * href="http://en.wikipedia.org/wiki/Daylight_saving_time">Daylight saving
	 * time</a> offset since this class is unaware of dates.
	 * 
	 * @return the offset in milliseconds not accounting for Daylight saving
	...
	{
		return (long) (getLongitude() * 4 * MINUTE_MILLIS - getTimeZone()
				.getRawOffset());
	}
	 ...
	 * href="http://en.wikipedia.org/wiki/Thaddeus_Vincenty">Thaddeus
	 * Vincenty's</a> inverse formula See T Vincenty, "<a
	 ...
	 *            the destination location
	 * @param formula
	 *            This formula calculates initial bearing (
	 *            {@link #INITIAL_BEARING}), final bearing (
	...
	/**
	 * Returns the <a href="http://en.wikipedia.org/wiki/Rhumb_line">rhumb
	 * line</a> bearing from the current location to the GeoLocation passed in.
	 * 
	 * @param location
	...
	public double getRhumbLineBearing(GeoLocation location)
	{
		...
		double dPhi = Math.log(Math.tan(Math.toRadians(location.getLatitude())
				/ 2 + Math.PI / 4)
	...
	/**
	 * Returns the <a href="http://en.wikipedia.org/wiki/Rhumb_line">rhumb
	 * line</a> distance from the current location to the GeoLocation passed in.
	 * 
	 * @param location
	 *            the destination location
	...
	public double getRhumbLineDistance(GeoLocation location)
	{
		...
		double dLon = Math.toRadians(Math.abs(location.getLongitude()
				- getLongitude()));
		double dPhi = Math.log(Math.tan(Math.toRadians(location.getLongitude())
				/ 2 + Math.PI / 4)
				/ Math.tan(Math.toRadians(getLatitude()) / 2 + Math.PI / 4));
		double q = (Math.abs(dLat) > 1e-10) ? dLat / dPhi : Math.cos(Math
				.toRadians(getLatitude()));
	 ...
	 * the serialized <code>Object</code>. Very similar to the toString method
	 * but the return value is in an xml format. The format currently used
	...
	public String toXML()
	{
		...
		sb.append("\t<LocationName>").append(getLocationName())
				.append("</LocationName>\n");
		sb.append("\t<Latitude>").append(getLatitude()).append("&deg;")
				.append("</Latitude>\n");
		sb.append("\t<Longitude>").append(getLongitude()).append("&deg;")
				.append("</Longitude>\n");
		sb.append("\t<Elevation>").append(getElevation()).append(" Meters")
				.append("</Elevation>\n");
		sb.append("\t<TimezoneName>").append(getTimeZone().getID())
				.append("</TimezoneName>\n");
		sb.append("\t<TimeZoneDisplayName>")
				.append(getTimeZone().getDisplayName())
				.append("</TimeZoneDisplayName>\n");
		sb.append("\t<TimezoneGMTOffset>")
				.append(getTimeZone().getRawOffset() / HOUR_MILLIS)
				.append("</TimezoneGMTOffset>\n");
		sb.append("\t<TimezoneDSTOffset>")
				.append(getTimeZone().getDSTSavings() / HOUR_MILLIS)
